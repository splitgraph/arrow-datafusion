...
/// cheaper than converting every row to a `ScalarValue` to test uniqueness. Returns
/// `None` if the type isn't dictionary-encodable or every value is null - both safe to
/// treat as "no pruning literals available".
